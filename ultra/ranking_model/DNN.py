...
class DNN(nn.Module):
    ...
    def __init__(self, hparams_str, feature_size):
        ...
        self.act_func = None
        self.output_sizes = self.hparams.hidden_layer_sizes + [self.hparams.output_size]
    ...
    def build(self, input_list, noisy_params=None,
        ...
        """
        input_data = torch.cat(input_list, dim=0) #2560*33
        input_data = input_data.to(dtype=torch.float32)
        if torch.cuda.is_available():
            input_data = input_data.to(device=device)
        #print(input_data.shape)
        if (noisy_params == None):
            output_data = self.sequential(input_data) #2560*1
        else:
        ...
        return torch.split(output_data, output_shape, dim=0)# 一个列表，包含10个256*1
